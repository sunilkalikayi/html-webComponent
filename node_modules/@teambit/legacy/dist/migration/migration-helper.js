...
function getMigrationVersions(currentVersion, storeVersion, migratonManifest, verbose = false) {
  ...
  const migrationsVersionsToRun = migrationsVersions.filter(version => _semver().default.satisfies(version, `>${storeVersion} <=${currentVersion}`));
  ...
  // migration process)
  const sortedMigrationVersionsToRun = migrationsVersionsToRun.sort(_semver().default.compare);
