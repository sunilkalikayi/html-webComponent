...
class FixtureHelper {
  ...
  addComponentUtilsIsTypeAsDir() {
    return this.command.addComponent('is-type', {
      i: 'utils/is-type'
    });
  }
  ...
  addComponentUtilsIsStringAsDir() {
    return this.command.addComponent('is-string', {
      i: 'utils/is-string'
    });
  }
  tagComponentBarFoo() {
  ...
  copyFixtureComponents(dir = '', dest = path().join(this.scopes.localPath, dir)) {
   ...
   */
  populateComponents(numOfComponents = 3, rewire = true, additionalStr = '', compile = true, esm = false) {
  ...
  getCjsImplForPopulate(numOfComponents, index, additionalStr = '') {
...
module.exports = () => 'comp${index}${additionalStr} and ' + ${nextComp}();`;
  ...
  getEsmImplForPopulate(numOfComponents, index, additionalStr = '') {
    if (index === numOfComponents) return `export default function(){
