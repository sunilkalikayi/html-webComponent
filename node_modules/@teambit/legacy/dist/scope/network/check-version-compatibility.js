...
function checkVersionCompatibility(remoteVersion) {
  ...
  if (remoteMajor > localMajor) {
    ...
    _logger().default.console(createMajorMessage(remoteVersion, _constants().BIT_VERSION), 'error');
  ...
  if (remoteMinor > localMinor) {
    _loader().default.stop();
    _logger().default.console(createMinorMessage(remoteVersion, _constants().BIT_VERSION), 'error');
    // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
    _loader().default.start();
    return;
  }
  if (remotePatch > localPatch) {
    _loader().default.stop();
    _logger().default.console(createMinorMessage(remoteVersion, _constants().BIT_VERSION), 'warn');
    // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
    _loader().default.start();
  }
}
...
function checkVersionCompatibilityOnTheServer(clientVersion) {
  ...
  const localMajor = semver().major(_constants().BIT_VERSION);
  const oldClientVersionMessageUntilV14 = `Please update your Bit client.\nFor additional information: https://${_constants().BASE_LEGACY_DOCS_DOMAIN}/installation#latest-version`;
...
For additional information: https://${_constants().BASE_DOCS_DOMAIN}/getting-started/installing-bit/installing-bit`;
