...
describe('detective-typescript', () => {
  ...
  describe('string in apostrophes', () => {
    it('should recognize when using require statement', () => {
      ...
      const depsKeys = Object.keys(deps);
      (0, _chai().expect)(depsKeys).to.have.lengthOf(1);
      (0, _chai().expect)(depsKeys[0]).to.equal('foo');
    });
  });
  describe('dynamic import', () => {
    it('should recognize regardless of the location', () => {
      const deps = (0, _().default)(`const loadable = (f) => f; const QuickStart = loadable(() => import('lodash'));`); // eslint-disable-line
      const depsKeys = Object.keys(deps);
      (0, _chai().expect)(depsKeys).to.have.lengthOf(1);
      (0, _chai().expect)(depsKeys[0]).to.equal('lodash');
    });
  });
