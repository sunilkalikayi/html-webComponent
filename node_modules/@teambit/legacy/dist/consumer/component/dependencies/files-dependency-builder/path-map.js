...
exports.getPathMapWithLinkFilesData = getPathMapWithLinkFilesData;
function _clone2() {
...
function convertPathMapToRelativePaths(pathMap, baseDir) {
  ...
  return pathMap.map(file => {
    const newFile = (0, _clone2().default)(file);
    ...
    newFile.dependencies = file.dependencies.map(dependency => {
      const newDependency = (0, _clone2().default)(dependency);
 ...
 */
function findTheRealDependency(allPathMapItems, firstPathMap, specifier) {
  let currentPathMap = firstPathMap;
  let lastRealDep;
  const visitedFiles = [];
  while (!visitedFiles.includes(currentPathMap.file)) {
    visitedFiles.push(currentPathMap.file);
    const currentRealDep = currentPathMap.dependencies.find(dep => {
 ...
 */
function getDependenciesFromLinkFileIfExists(dependency, pathMap) {
  const dependencyPathMap = pathMap.find(file => file.file === dependency.resolvedDep);
  if (!dependencyPathMap || !dependencyPathMap.dependencies.length || !dependency.importSpecifiers || !dependency.importSpecifiers.length) {
    return null;
  }
  const dependencies = dependency.importSpecifiers.map(specifier => {
  ...
  dependencies.forEach(dep => {
    ...
    if (existingFile) {
      ...
      linkFiles.push({
        file: dep.file,
        importSpecifiers: [dep.importSpecifier]
      });
    }
  });
  return linkFiles;
 ...
 */
function getPathMapWithLinkFilesData(pathMap) {
  if (!Array.isArray(pathMap)) throw new TypeError('pathMap must be an array');
  const updateDependencyWithLinkData = dependency => {
