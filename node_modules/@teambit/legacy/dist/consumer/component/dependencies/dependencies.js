...
class Dependencies {
  ...
  cloneAsString() {
    return this.dependencies.map(dependency => {
      const dependencyClone = (0, _clone2().default)(dependency);
   ...
   */
  getSourcesPaths() {
    return (0, _flatten2().default)(this.dependencies.map(dependency => dependency.relativePaths.map(relativePath => {
  ...
  async addRemoteAndLocalVersions(scope, modelDependencies) {
    ...
    const remoteVersionsDependencies = await (0, _scopeRemotes().fetchRemoteVersions)(scope, dependenciesIds);
    this.dependencies.forEach(dependency => {
      ...
      const modelVersionId = modelDependencies.get().find(modelDependency => modelDependency.id.isEqualWithoutVersion(dependency.id));
      // @ts-ignore AUTO-ADDED-AFTER-MIGRATION-PLEASE-FIX!
  ...
  validate(bitId) {
    ...
    this.dependencies.forEach(dependency => {
      ...
      if (bitId && bitId.isEqual(dependency.id)) {
        throw new (_validationError().default)(`failed validating ${bitId.toString()}, one of the dependencies has the same id as the component`);
      }
      ...
      dependency.relativePaths.forEach(relativePath => {
        ...
        if (relativePath.importSpecifiers) {
          (0, _validateType().default)(message, relativePath.importSpecifiers, 'relativePath.importSpecifiers', 'array');
